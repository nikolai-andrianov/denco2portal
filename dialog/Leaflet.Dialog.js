L.Control.Dialog = L.Control.extend({
  ...
  // Called when a new instance of the dialog is created via var dialog = L.control.dialog() 
  // Set the dialog options to the ones above
  ...
  // Called via .setContent(contents) where content is HTML to be placed in the dialog
  ...
  // First called from setContent() when no map is present, so just returns
  // Then called from onAdd
  update: function() {
    if (!this._map) {
      return;
    }
  ...
  // Called when the dialog window is moved
  ...
  // Called when the mouse is unclicked after moving the dialog window
